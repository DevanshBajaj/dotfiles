...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _PBCombinedMergedStorage = __webpack_require__(8);

	var _PBCombinedMergedStorage2 = _interopRequireDefault(_PBCombinedMergedStorage);

	var _PBCombinedMappedStorage = __webpack_require__(10);
	...
	var PBStorage = function () {
	  function PBStorage(globalStore, storage) {
	    ...
	    this.pb_reportData = this.createStorage("pb_reportData");
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _PBCombinedStorage2 = __webpack_require__(9);
...
/***/ (function(module, exports) {
	...
	var Config = {
	  ...
	  "defaultWhiteList": ["linkedin.com", "google", "www.gmail.com", "www.pinterest.com", "www.youtube.com", "www.facebook.com", "search.yahoo.com", "chrome://newtab", "www.food.com"],
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _overlayKiller = __webpack_require__(19);

	var _overlayKiller2 = _interopRequireDefault(_overlayKiller);

	var _inject = __webpack_require__(21);

	var _ntfShowManager = __webpack_require__(22);

	var _ntfShowManager2 = _interopRequireDefault(_ntfShowManager);

	var _notification = __webpack_require__(23);

	var _notification2 = _interopRequireDefault(_notification);

	var _blockCountManager = __webpack_require__(24);

	var _blockCountManager2 = _interopRequireDefault(_blockCountManager);
	...
	(function ($) {
	  var pbMessage = (0, _utils.randId)();

	  var ntfShowManager = new _ntfShowManager2.default();

	  var blockCountManager = new _blockCountManager2.default();

	  var notification = new _notification2.default({
	    onHideClick: function onHideClick(_ref) {
	      var source = _ref.source;

	      ntfShowManager.updateDontShow({ domain: document.domain });

	      sendGoogleAnalyticsEvent({ category: 'Notification_bar', action: 'Hide' });
	    },
	    onResizeClick: function onResizeClick(_ref2) {
	      var size = _ref2.size;

	      sendGoogleAnalyticsEvent({ category: 'Notification_bar', action: 'Resize', label: size });
	    },
	    onDisplay: function onDisplay(_ref3) {
	      var source = _ref3.source;

	      switch (source) {
	        case 'popup-blocked':
	          sendGoogleAnalyticsEvent({ category: 'Notification_bar', action: 'Shown' });
	          break;

	        case 'recipe-blocked':
	          sendGoogleAnalyticsEvent({ category: 'Notification_bar_overlay', action: 'Shown' });
	          break;
	        ...
	        case 'overlay-not-found':
	          sendGoogleAnalyticsEvent({ category: 'Not_found_notification', action: 'Shown', label: document.domain });
	          sendMessageToBackground({ name: 'KeenIO', data: { event: 'overlay-ntf:not-found', post: { 'domain': document.domain, 'url': location.href } } });
	          break;

	        case 'rating':
	          sendGoogleAnalyticsEvent({ category: 'rankUs', action: 'Shown', label: document.domain, isCount: true });
	          break;

	        case 'quick-tip':
	          sendGoogleAnalyticsEvent({ category: 'tip_notification', action: 'Shown', label: document.domain });
	          break;
	      }
	    },
	    onCloseClick: function onCloseClick(_ref4) {
	      var source = _ref4.source;

	      switch (source) {
	        case 'popup-blocked':
	          sendGoogleAnalyticsEvent({ category: 'Notification_bar', action: 'Click', label: 'Close', domain: domain });
	          break;

	        case 'recipe-blocked':
	          sendGoogleAnalyticsEvent({ category: 'Notification_bar_overlay', action: 'Click', label: 'Close' });
	          break;
	        ...
	        case 'overlay-not-found':
	          sendGoogleAnalyticsEvent({ category: 'Not_found_notification', action: 'Click', label: 'Close' });
	          break;

	        case 'rating':
	          sendGoogleAnalyticsEvent({ category: 'rankUsClose', action: 'Click', label: 'Close' });
	          break;

	        case 'share':
	          sendGoogleAnalyticsEvent({ category: 'shareUsClose', action: 'Click', label: 'Close' });
	          break;

	        case 'quick-tip':
	          sendGoogleAnalyticsEvent({ category: 'tip_notification', action: 'Click', label: 'Close' });
	          break;
	      }
	    },
	    onButtonClick: function onButtonClick(_ref5) {
	      ...
	      switch (source) {
	        case 'popup-blocked':
	          switch (id) {
	            case 'allowOnce':
	              sendGoogleAnalyticsEvent({ category: 'Notification_bar', action: 'Click', label: 'Allow once' });
	              //sendMessageToBackground({name:'KeenIO', data:{event: 'block-pop-ntf:allow-once', post: {'domain': document.domain, 'url': location.href}}});
	              break;

	            case 'allowAlways':
	              sendGoogleAnalyticsEvent({ category: 'Notification_bar', action: 'Click', label: 'Allow always' });
	              //sendMessageToBackground({name:'KeenIO', data:{event: 'block-pop-ntf:allow-always', post: {'domain': document.domain, 'url': location.href}}});
	              break;
	          }
	          break;
	        ...
	        case 'recipe-blocked':
	          switch (id) {
	            case 'NTF_allowOverlayOnce':
	              sendGoogleAnalyticsEvent({ category: 'Notification_bar_overlay', action: 'Click', label: 'Allow once' });
	              break;
	          }
	          break;
	        ...
	        case 'removed-overlay':
	          switch (id) {
	            case 'undoLabel':
	              sendGoogleAnalyticsEvent({ category: "Notification_bar_overlay", action: "Click", label: "Undo Once" });
	              sendMessageToBackground({ name: 'KeenIO', data: { event: 'overlay-ntf:undo', post: { 'domain': document.domain, 'url': location.href } } });
	              break;

	            case 'removeAlwaysLabel':
	              sendGoogleAnalyticsEvent({ category: "Notification_bar_overlay", action: "Click", label: "remove Always" });
	              break;
	          }
	          break;
	        ...
	        case 'overlay-blacklist':
	          switch (id) {
	            ...
	            case 'NTF_whitelistOverlay':
	              sendGoogleAnalyticsEvent({ category: 'Notification_bar_overlay', action: 'Click', label: 'Allow always' });
	              break;
	          }
	          break;
	        ...
	        case 'overlay-not-found':
	          switch (id) {
	            case 'TS_Report':
	              sendGoogleAnalyticsEvent({ category: 'Not_found_notification', action: 'Click', label: 'Report' });
	              sendMessageToBackground({ name: 'KeenIO', data: { event: 'report-overlay-not-found', post: { 'domain': document.domain, 'url': location.href } } });
	              break;
	          }
	          break;
	      }
	  ...
	  var overlayKiller = new _overlayKiller2.default({
	    onUserRemove: function onUserRemove(_ref6) {
	      var success = _ref6.success,
	          _ref6$recepies = _ref6.recepies,
	          recepies = _ref6$recepies === undefined ? [] : _ref6$recepies,
	          _ref6$props = _ref6.props,
	          props = _ref6$props === undefined ? {} : _ref6$props;

	      if (success) {
	        showOverlayRemovedNotifiction({ recepies: recepies, props: props });

	        updateBlockCounts({ block: 'overlay' });
	      } else {
	        showNoOverlayFoundNotification();
	      }
	    },
	    onRecipeFound: function onRecipeFound(_ref7) {
	      var found = _ref7.found;

	      ntfShowManager.isShow({ domain: document.domain, type: 'overlay' }).then(function () {
	        showRecipeOverlayNortification();
	      });

	      updateBlockCounts({ block: 'overlay' }, found.length);
	    },
	    onBlockListFound: function onBlockListFound(_ref8) {
	      var found = _ref8.found;

	      ntfShowManager.isShow({ domain: document.domain, type: 'overlay' }).then(function () {
	        showBlockListOverlayNortification();
	      });

	      updateBlockCounts({ block: 'overlay' }, found.length);
	    },
	    onRecipeNotFound: function onRecipeNotFound(notFound) {
	      notFound.forEach(function (item) {
	        //sendToBackground('KeenIO', {event: 'overlay-recipe-not-found', post: {'domain': document.domain, 'url': location.href, 'recipe': btoa(item.recipe)}});
	      });
	    }
	  });

	  var uninstallMonitor = {
	    isMonitor: true,
	    uninstallCheckRate: 3000,
	    inlineUninstallURL: "https://www.poperblocker.com/uninstall/inlineUninstall.html"
	  };

	  if (window == top) {
	    chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
	      switch (request.name) {
	        //from bg after recepies callback
	        case 'receivedRecipes':
	          var recepies = (request.data || {}).recipes || [];

	          if (recepies.length) {
	            overlayKiller.createBlockListFromRecepies(recepies);
	          }
	          break;

	        case 'manual-remove-overlay':
	          overlayKiller.remove({ source: request.data.source });
	          break;
	        case "closePopupMenu":
	          closePopupMenu();
	          break;
	        case "setUninstallMonitor":
	          setUninstallMonitor(request.data);
	          break;
	        case "offUninstallMonitor":
	          if (window.uninstallMonitorInterval) {
	            clearInterval(window.uninstallMonitorInterval);
	          }
	          break;
	        case "getReportData":
	          sendResponse({ referrer: document.referrer });
	          break;
	        // toggle the function, not working in this version
	        case "toggleFunction":
	          if (request.toggle) {
	            initBlockFunctions();
	          } else {
	            (0, _utils.executeCode)("window.pbExternalCommand(0, \"" + pbMessage + "\");");
	          }
	          break;
	        case 'showRateUs':
	          showRateUsPopup();
	  ...
	  parsePendingNotificationParams();
	      ...
	      if (event.data.type && event.data.type == "blockedWindow") {
	        var _ret = function () {
	          if (self !== top) {
	            ...
	            parent.postMessage({ type: "blockedWindow", args: event.data.args }, parentOrigin);
	            return {
	              v: void 0
	            };
	          }
	          ...
	          updateBlockCounts({ block: 'pop', args: args });

	          ntfShowManager.isShow({ domain: document.domain }).then(function () {
	            showPopupBlockedNotification(args, { domain: document.domain });
	          });

	          if (ntfShowManager.isActive) {
	            setPendingNotificationParams(args);
	          }

	          sendGoogleAnalyticsEvent({ category: "General", action: "Blocked_popups", label: document.domain });
	        }();
	  ...
	  //show notifcation after a force redirect of a site
	  //every time a popup is being block this function runs
	  //args = popup details (url, name, props)
	  function setPendingNotificationParams(args) {
	    var pop = (args[0] || '') + "|" + (args[1] || '') + "|" + (args[2] || '');
	    var relatedLink = args[3] || '';
	    var currentHost = location.host;
	    var popHost = new URL((0, _utils.getAbsoluteURL)(args[0])).host;
	    //check if the popup is the same host that the current domain
	    //this flags this as a potential pattern of: open the site's page as a popup and redirect to an ad
	    var isSameHostPopup = popHost == currentHost;
	    //check if ad block found - this makes sites to be more agreesive with popups
	    var abd = args['abd'] ? 1 : 0;
	    var data = {
	      pop: pop,
	    ...
	    //if same url popup is detected and the user creates a "redirect"
	    //by clicking a link, this should prevent to send the user back to the origin site
	    if (data.isSameHostPopup) {
	  ...
	  function parsePendingNotificationParams() {
	    if (window.name && window.name.indexOf('pp_pending_ntf:') == 0) {
	      (function () {
	        var data = JSON.parse(atob(name.split(':')[1]));
	        var fnRemove = function fnRemove() {
	          removePendingNotificationParams();

	          $(document).off('click', fnRemove);
	        };

	        //max 5000 ms between popup block and page load
	        if (data.time + 5000 >= Date.now()) {
	          //force display the notification
	          if (data.display) {
	            ntfShowManager.isShow({ domain: document.domain }).then(function () {
	              showPopupBlockedNotification(data.pop.split('|'), { preventDisplayEvent: true, quickOpen: true, domain: document.domain });
	            });
	            ...
	            (function () {
	              ...
	              //self popup + redirect pattern - return user to the popup url and block the redirected site
	              if (isExternalSite && data.abd) {
	                var url = data.pop.split('|')[0];

	                if (data.isSameHostPopup) {
	                  setSingleDisplayNotificationParams();

	                  location.href = url;
	                } else if (/^about:blank/i.test(url) && data.relatedLink) {
	                  setSingleDisplayNotificationParams();

	                  location.href = data.relatedLink;
	                }
	              } else {
	                ntfShowManager.isShow({ domain: isExternalSite ? data.host : document.domain }).then(function () {
	                  showPopupBlockedNotification(data.pop.split('|'), { preventDisplayEvent: true, quickOpen: true, domain: isExternalSite ? data.host : document.domain });
	                });
	  ...
	  function showPopupBlockedNotification(args) {
	    ...
	    notification.show({
	      title: (0, _utils.getI18N)('notificationMessage'),
	      subTitle: (0, _utils.getI18N)('NTF_ON_SITE') + ' ' + domain,
	      quickOpen: props.quickOpen,
	      showHide: true,
	      showResize: true,
	      source: 'popup-blocked',
	      winArgs: args, //pop url / name / width + height
	      onDisplay: function onDisplay() {
	        if (!props.preventDisplayEvent) {
	          sendMessageToBackground({ name: 'update-rating', action: 'display' });
	        }
	      },
	      onButtonClick: function onButtonClick() {
	        sendMessageToBackground({ name: 'update-rating', action: 'click' });
	      },
	      ...
	      buttons: {
	        allowOnce: function allowOnce() {
	          ntfShowManager.isActive = true;
	        },
	        allowAlways: function allowAlways() {
	          sendMessageToBackground({ name: 'allowAlways', domain: domain, type: 'popup' });
	  ...
	  //an overlay blocked by recipe notification
	  function showRecipeOverlayNortification() {
	    notification.show({
	      title: (0, _utils.getI18N)("overlayWasBlocked"),
	      subTitle: (0, _utils.getI18N)('NTF_ON_SITE') + ' ' + document.domain,
	      icon: chrome.runtime.getURL("/images/logo-20x20.svg"),
	      source: 'recipe-blocked',
	      showHide: true,
	      showResize: true,
	      buttons: {
	        NTF_allowOverlayOnce: function NTF_allowOverlayOnce() {
	  ...
	  //overlay blocked by user and added to blacklist
	  function showBlockListOverlayNortification() {
	    notification.show({
	      title: (0, _utils.getI18N)('overlayWasBlocked'),
	      subTitle: (0, _utils.getI18N)('NTF_ON_SITE') + ' ' + document.domain,
	      icon: chrome.runtime.getURL('/images/logo-20x20.svg'),
	      showHide: true,
	      showResize: true,
	      source: 'overlay-blacklist',
	      buttons: {
	        NTF_allowOverlayOnce: function NTF_allowOverlayOnce() {
	          overlayKiller.restore();
	        },
	        NTF_whitelistOverlay: function NTF_whitelistOverlay() {
	          overlayKiller.restore();
	          //user blacklisted this overlay so we need to remove it from blacklist
	          overlayKiller.removeFromLocalBlacklist();
	        }
	      },
	      onDisplay: function onDisplay() {
	        sendMessageToBackground({ name: 'update-rating', action: 'display' });
	  ...
	  //overlay removed by user right click (or from popup menu)
	  function showOverlayRemovedNotifiction(_ref9) {
	    var recepies = _ref9.recepies,
	        source = _ref9.props.source;

	    notification.show({
	      title: (0, _utils.getI18N)('overlayWasRemoved'),
	      subTitle: (0, _utils.getI18N)("NTF_ON_SITE") + ' ' + document.domain,
	      icon: chrome.runtime.getURL('/images/logo-20x20.svg'),
	      showResize: true,
	      source: 'removed-overlay',
	      buttons: {
	        undoLabel: function undoLabel() {
	          overlayKiller.restore();

	          ntfShowManager.isActive = true;
	        },
	        removeAlwaysLabel: function removeAlwaysLabel() {
	          overlayKiller.addToLocalBlacklist();

	          sendMessageToBackground({ name: 'KeenIO', data: { event: 'overlay-ntf:remove-always', post: { 'domain': document.domain, 'url': location.href, recepies: btoa(recepies) } } });
	        }
	      },
	      onDisplay: function onDisplay() {
	        sendMessageToBackground({ name: 'update-rating', action: 'display' });
	      ...
	      onClose: function onClose() {
	        if (source == 'menu') {
	          ntfShowManager.isShow({ domain: document.domain, type: 'tip' }).then(function () {
	            showOverlayTipFromMenu();
	          });
	        }
	      }
	    });
	  ...
	  //no overlay found notification
	  function showNoOverlayFoundNotification() {
	    notification.show({
	      title: (0, _utils.getI18N)('TS_NoOverlay'),
	      subTitle: (0, _utils.getI18N)('NTF_ON_SITE') + ' ' + document.domain,
	      icon: chrome.runtime.getURL('/images/logo-20x20.svg'),
	      source: 'overlay-not-found',
	      size: 'maximized',
	      buttons: {
	        TS_Report: function TS_Report() {
	          reportNoOverlayFound(location.href, location.host);
	        }
	      }
	    });
	  }
	  ...
	  function showOverlayTipFromMenu() {
	    notification.addQueue({
	      stopQueue: true,
	      title: (0, _utils.getI18N)('NTF_QUICK_TIP'),
	      subTitle: (0, _utils.getI18N)('NTF_TIP_OVERLAY_MENU'),
	      icon: chrome.runtime.getURL('/images/ntf/bulb.svg'),
	      size: 'maximized',
	      source: 'quick-tip'
	    });
	  }
	  ...
	  function showRateUsPopup() {
	    notification.addQueue({
	      title: (0, _utils.getI18N)("rateus"),
	      subTitle: (0, _utils.getI18N)("TS_Rate_ShowLove"),
	      icon: false,
	      persistent: true,
	      showStars: true,
	      size: 'maximized',
	      cssClass: 'pb-toast-large',
	      source: 'rating',
	      onRating: function onRating(score) {
	        if (score == 5) {
	          openUrl(_const2.default['extension_review_page_' + (0, _utils.getBrowser)()]);
	        } else {
	          openUrl(_const2.default.contact_page);
	        }

	        sendGoogleAnalyticsEvent({ category: 'rankUs', action: 'Click', label: score + '_stars', isCount: true });
	        sendMessageToBackground({ name: 'update-rating', action: 'complete' });
	      }
	    });
	  }

	  function updateBlockCounts(blockInfo) {
	    var counts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;

	    blockCountManager.updateCounts({ blockInfo: blockInfo, counts: counts }).then(function (data) {
	      sendMessageToBackground({ name: "updateBlockCounts", numOfBlocks: data.numOfBlocks });

	      _PBStorageSync2.default.pb_hideBadge.get().then(function (show) {
	        if (!show) {
	          sendMessageToBackground({ name: "setBadgeText" });
	        }
	      });
	    });
	  }

	  function reportNoOverlayFound(tabUrl, host) {
	    sendMessageToBackground({
	      name: "reportNoOverlayFound",
	      data: { url: tabUrl, host: host, message: 'Report Overlay' }
	    });
	  }

	  function sendGoogleAnalyticsEvent(data) {
	    var _data$domain = data.domain,
	        domain = _data$domain === undefined ? document.domain : _data$domain;
	  ...
	  function setUninstallMonitor(info) {
	    if (window.uninstallMonitorInterval) {
	      clearInterval(window.uninstallMonitorInterval);
	    }
	    window.uninstallMonitorInterval = setInterval(function () {
	      if (!isExtensionAlive() && $("#poperblockerInlineInstall").length === 0) {
	        var iframe = document.createElement('iframe');
	        iframe.style.display = "none";
	        iframe.style.height = "500px";
	        iframe.style.width = "500px";
	        iframe.id = "poperblockerInlineInstall";
	        iframe.src = uninstallMonitor.inlineUninstallURL + "?version=" + info.version + "&block=" + info.blockCounts || 0;
	        document.body.appendChild(iframe);
	        clearInterval(window.uninstallMonitorInterval);
	      }
	    }, uninstallMonitor.uninstallCheckRate);
	  }

	  function initBlockFunctions() {
	    _PBStorageSync2.default.many(_PBStorageSync2.default.pb_hideBadge, _PBStorageSync2.default.pb_fullWhitelist, _PBStorageSync2.default.pb_overlaylist, _PBStorageSync2.default.pb_pause, _PBStorageSync2.default.pb_popupBlackList).get().then(function (settings) {
	      //paused do not do anything
	      if (settings.pb_pause) {
	        return;
	      }

	      var whitelist = settings.pb_fullWhitelist;

	      if (!(0, _utils.isDomainInList)(document.domain, whitelist)) {
	        ...
	        (0, _utils.executeCode)(code);

	        /*addScript({textContent: `${inject.toString()}`, id: "pb_blockScript"});
	        executeCode("var pb_blacklist = " + JSON.stringify(settings.pb_popupBlackList));
	        executeCode("var pb_whitelist = " + JSON.stringify(whitelist));
	        executeCode("inject()");*/
	  ...
	  // blocking popups - START
	  _PBStorageSync2.default.pb_fullWhitelist.get().then(function (whitelist) {
	    //get whitelist
	    if (!(0, _utils.isDomainInList)(document.domain, whitelist)) {
	  ...
	  function isOverlayish(el) {
	    ...
	    if (el.outerWidth() >= winWidth * 0.6 && el.outerHeight() >= winHeight * 0.75) {
	      return true;
	    }
	  ...
	  function onHyperLinkClicked(e) {
	    ...
	    if (isExtensionAlive() && href && href !== '#' && target !== '_self') {
	  ...
	  function isExtensionAlive() {
	    try {
	      return (0, _utils.getI18N)("options");
	    } catch (e) {
	      return false;
	    }
	  }

	  function openUrl(url, notNewTab) {
	    sendMessageToBackground({ name: "openUrl", url: url, notNewTab: notNewTab });
	  }
	...
	if (window == top) {
	  setTimeout(function () {
	    window.onfocus = function () {
	      sendMessageToBackground({ name: "page", url: window.location.href });
	    };
	  }, 2000);

	  window.onbeforeunload = function () {
	    sendMessageToBackground({ name: "pageClosing", url: window.location.href });
	    return null;
	  };
	}
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var OverlayDectector = function () {
	  function OverlayDectector() {
	    _classCallCheck(this, OverlayDectector);

	    this.winWidth = window.innerWidth;
	    this.winHeight = window.innerHeight;
	  }

	  _createClass(OverlayDectector, [{
	    key: 'ALGO1',
	    value: function ALGO1() {
	      var _this = this;

	      var children = document.querySelectorAll('body > :not(script)');
	      var center = { x: this.winWidth / 2, y: this.winHeight / 2 };
	      var candidates = [],
	          winner = void 0;
	      var overlay = void 0;

	      children.forEach(function (item) {
	        var res = _this.isOverlayish(item);
	      ...
	      if (candidates.length) {
	        winner = candidates.sort(function (a, b) {
	          return b[1] - a[1];
	        })[0];
	        ...
	        if (winner) {
	          ...
	          if (center.x > winner.offsetLeft && center.x < winner.offsetLeft + winner.offsetWidth && center.y > winner.offsetTop && center.y < winner.offsetTop + winner.offsetHeight) {
	            overlay = winner;
	          }
	        }
	      }
	    ...
	    value: function ALGO2(_ref) {
	      ...
	      if (element && this.notRootElement(element)) {
	        do {
	          var isOverlayish = this.isOverlayish({ el: element, minWidth: mw, minHeight: mh });
	    ...
	    key: 'notRootElement',
	    value: function notRootElement(element) {
	      return element != document.body && element != document && element != document.documentElement;
	    }
	  }, {
	    key: 'detectOverlayLayer',
	    value: function detectOverlayLayer(detectRange) {
	      var _iteratorNormalCompletion = true;
	      var _didIteratorError = false;
	      var _iteratorError = undefined;

	      try {
	        for (var _iterator = detectRange[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	          var coords = _step.value;

	          var overlay = this.ALGO2(coords);

	          if (overlay) {
	            return this.getRecipe(overlay);
	          }
	        }
	      } catch (err) {
	        _didIteratorError = true;
	    ...
	    value: function find() {
	      var detect = {
	        AnySize: [{ x: this.winWidth / 2, y: this.winHeight / 2, mw: 100, mh: 100 }, { x: this.winWidth / 2, y: this.winHeight / 3, mw: 100, mh: 100 }],
	        FullScreen: [{ x: this.winWidth / 2, y: this.winHeight / 2, mw: this.winHeight * 0.9, mh: this.winHeight * 0.9 }]
	      };
	      var overlays = [],
	          recipe = void 0;

	      recipe = this.detectOverlayLayer(detect.AnySize);

	      if (recipe) {
	        overlays.push(this.hideByRecipe(recipe));

	        recipe = this.detectOverlayLayer(detect.FullScreen);
	    ...
	    key: 'getRecipe',
	    value: function getRecipe(el) {
	      var recipe = [];
	      var isSingleInstanceInDocument = function isSingleInstanceInDocument(recipe) {
	        return (0, _jQuery2.default)(recipe.join(' > ')).length == 1;
	      };
	      ...
	      do {
	        ...
	        if (id) {
	          ...
	          recipe.unshift(node + '.' + className.trim().replace(/\s+/g, '.'));
	        } else {
	          recipe.unshift(node + '[style="' + style + '"]');
	        }

	        if (isSingleInstanceInDocument(recipe)) {
	          break;
	        }
	    ...
	    key: 'hideByRecipe',
	    value: function hideByRecipe(recipe) {
	      var el = (0, _jQuery2.default)(recipe);
	      var css = el.prop('style').cssText;

	      el.prop('style').cssText = css + ';display:none !important;';

	      return { el: el, recipe: recipe, css: css };
	    }
	  }, {
	    key: 'isOverlayish',
	    value: function isOverlayish(_ref2) {
	      var el = _ref2.el,
	      ...
	      if (css.display != 'none' && css.visibility != 'hidden' && /fixed|absolute/.test(css.position) && (css.zIndex == 'auto' || parseInt(css.zIndex) >= 0) && el.offsetWidth >= minWidth && el.offsetHeight >= minHeight) {
	        return [el, css.zIndex * 1];
	      }
	  ...
	  return OverlayDectector;
	}();

	var OverlayKiller = function (_OverlayDectector) {
	  _inherits(OverlayKiller, _OverlayDectector);

	  function OverlayKiller() {
	    var props = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

	    _classCallCheck(this, OverlayKiller);

	    var _this2 = _possibleConstructorReturn(this, (OverlayKiller.__proto__ || Object.getPrototypeOf(OverlayKiller)).call(this));

	    _this2.activeBlockList = [];
	    _this2.activeRecepiesList = [];

	    _this2.onAutoDetect = props.onAutoDetect || function () {};
	    _this2.onUserRemove = props.onUserRemove || function () {};
	    _this2.onBlockListFound = props.onBlockListFound || function () {};
	    _this2.onRecipeFound = props.onRecipeFound || function () {};
	    _this2.onRecipeNotFound = props.onRecipeNotFound || function () {};

	    _this2.blackListPromise = _this2.createBlockListFromStorage();
	  ...
	  _createClass(OverlayKiller, [{
	    key: 'hide',
	    value: function hide(recepies) {
	      var _this3 = this;

	      this.last = [];

	      recepies.forEach(function (recipe) {
	        _this3.last.push(_this3.hideByRecipe(recipe));
	      });
	    }
	  }, {
	    key: 'restore',
	    value: function restore() {
	      this.last.forEach(function (item) {
	        ...
	        el.prop('style').cssText = css + ';display: block';

	        [].concat(_toConsumableArray((0, _jQuery2.default)('style.pp-remove-overlay'))).forEach(function (style) {
	          style = (0, _jQuery2.default)(style);

	          if (style.data('recipe') == btoa(recipe)) {
	            (0, _jQuery2.default)(style).remove();
	          }
	        });
	      });

	      window.dispatchEvent(new Event('resize')); //sometimes this makes the overlay to be centered properly when restored
	    }

	    // ======= remove overlay triggered by user ==========
	    ...
	    value: function remove() {
	      ...
	      var overlays = this.find();

	      if (overlays.length) {
	        this.last = overlays;

	        this.createGlobalCSS();
	        this.onUserRemove({ success: true, recepies: overlays.map(function (o) {
	            return o.recipe;
	          }), props: props });
	      } else {
	        this.onUserRemove({ success: false, props: props });
	      }
	    }

	    // ===== add found overlay to local blacklist, <remove always> button from notification

	  }, {
	    key: 'addToLocalBlacklist',
	    value: function addToLocalBlacklist() {
	      var _this4 = this;

	      var domain = document.domain;
	      var url = location.href;

	      _PBStorageSync2.default.pb_overlayBlockedList.update(function () {
	        var list = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

	        var blocks = list[domain] || [];

	        blocks.push({
	          id: 'block-' + blocks.length,
	          url: url,
	          domain: domain,
	          recipe: _this4.last.map(function (item) {
	            return item.recipe;
	          }).join(', ')
	        });
	    ...
	    // ===== remove overlay from local blacklist

	  }, {
	    key: 'removeFromLocalBlacklist',
	    value: function removeFromLocalBlacklist() {
	      var _this5 = this;
	      ...
	      _PBStorageSync2.default.pb_overlayBlockedList.update(function () {
	        ...
	        var blocks = list[domain] || [];
	        var lastRecepies = _this5.last.map(function (item) {
	          return item.recipe;
	        });

	        blocks = blocks.filter(function (item) {
	          return lastRecepies.indexOf(item.recipe) == -1;
	        });
	    ...
	    value: function autoDetect() {
	      ...
	      if (attempt < 40) {
	        //let overlay = this.find();
	        var foundBlockList = this.forceBlock(this.activeBlockList);
	        var foundRecipeList = this.forceBlock(this.activeRecepiesList);

	        /*if (overlay && !this.autoDetected) {
	          let recipe = btoa(this.getRecipe(overlay));
	            this.onAutoDetect(recipe);
	          this.autoDetected = true;
	        }*/

	        if (foundBlockList.length) {
	          this.onBlockListFound({ found: foundBlockList });
	        }

	        if (foundRecipeList.length) {
	          this.onRecipeFound({ found: foundRecipeList });
	        }

	        setTimeout(function () {
	          _this6.autoDetect(++attempt);
	        }, 250);
	      } else {
	        this.checkRecepiesSuccess();
	      }
	    }

	    //auto block all overlays from local db

	  }, {
	    key: 'createBlockListFromStorage',
	    value: function createBlockListFromStorage() {
	      var _this7 = this;
	      ...
	      return new _Promise(function (resolve) {
	        _PBStorageSync2.default.pb_overlayBlockedList.get().then(function () {
	          ...
	          if (list[document.domain]) {
	            _this7.activeBlockList = list[document.domain];

	            _this7.createBlockCSS(_this7.activeBlockList);
	          }
	    ...
	    //block all overlays found in recepies

	  }, {
	    key: 'createBlockListFromRecepies',
	    value: function createBlockListFromRecepies(recepies) {
	      var _this8 = this;

	      //local blacklist needs to be loaded before handeling recepies
	      this.blackListPromise.then(function () {
	        var blockList = _this8.activeBlockList.map(function (item) {
	          return item.recipe;
	        });

	        recepies = recepies.map(function (recipe) {
	          return {
	            recipe: recipe,
	            found: false
	          };
	        }).filter(function (item) {
	          return blockList.indexOf(item.recipe) == -1;
	        });

	        _this8.activeRecepiesList = recepies;

	        _this8.createBlockCSS(recepies);
	      });
	    }

	    //checks what recepies were found / not found and report

	  }, {
	    key: 'checkRecepiesSuccess',
	    value: function checkRecepiesSuccess() {
	      var notFound = this.activeRecepiesList.filter(function (item) {
	        return !item.found;
	      });

	      if (notFound.length) {
	        this.onRecipeNotFound(notFound);
	      }
	    }
	  }, {
	    key: 'createGlobalCSS',
	    value: function createGlobalCSS() {
	      var htmlStyle = (0, _jQuery2.default)('html').attr('style');
	      var bodyStyle = (0, _jQuery2.default)('body').attr('style');

	      (0, _jQuery2.default)('<style type="text/css">html, body {overflow:auto !important; }</style>').appendTo('head');
	      (0, _jQuery2.default)('html').attr('style', (htmlStyle ? htmlStyle + ';' : '') + 'overflow:auto !important;');
	      (0, _jQuery2.default)('body').attr('style', (bodyStyle ? bodyStyle + ';' : '') + 'overflow:auto !important;');
	    }

	    //blocks overlays with css for faster blocking

	  }, {
	    key: 'createBlockCSS',
	    value: function createBlockCSS(list) {
	      this.createGlobalCSS();

	      list.forEach(function (item) {
	        var css = item.recipe + ' { display:none !important; }';

	        (0, _jQuery2.default)('<style class="pp-remove-overlay" data-recipe="' + btoa(item.recipe) + '" type="text/css">' + css + '</style>').appendTo('head');
	      });
	    }

	    // ========= if block by css doesn't work, this will work, but only after DOM ready

	  }, {
	    key: 'forceBlock',
	    value: function forceBlock(list) {
	      var hide = [];

	      list.filter(function (item) {
	        return !item.found;
	      }).forEach(function (item) {
	        var overlay = (0, _jQuery2.default)(item.recipe);

	        if (overlay.length) {
	          item.found = true;

	          hide.push(item.recipe);
	        }
	      });
	  ...
	  return OverlayKiller;
	}(OverlayDectector);

	exports.default = OverlayKiller;
...
/***/ (function(module, exports) {
	...
	exports.restoreWindowOpen = restoreWindowOpen;
	/**
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var NtfShowManager = function () {
	  function NtfShowManager() {
	    _classCallCheck(this, NtfShowManager);

	    this.isActive = { pop: true, overlay: true, tip: true };
	  }

	  _createClass(NtfShowManager, [{
	    key: 'isShow',
	    value: function isShow() {
	      ...
	      var domain = props.domain,
	          ...
	          type = _props$type === undefined ? 'pop' : _props$type;


	      return new _Promise(function (resolve) {
	        _PBStorageSync2.default.many(_PBStorageSync2.default.pb_hideNotifications, _PBStorageSync2.default.doNotShowNotifyList, _PBStorageSync2.default.pb_lastNotificationDisplay).get().then(function (settings) {
	          var isGlobalHidden = settings.pb_hideNotifications;
	          var hiddenSitesList = settings.doNotShowNotifyList;
	          var lastDisplay = settings.pb_lastNotificationDisplay || {};

	          if (_this.isActive[type] && _this.checkLastDisplay({ domain: domain, type: type, lastDisplay: lastDisplay }) && !isGlobalHidden && !_this.checkIsHidden({ hiddenSitesList: hiddenSitesList, domain: domain })) {
	            _this.isActive[type] = false;

	            _this.updateLastDisplay({ domain: domain, type: type });
	            resolve();
	          }
	        });
	      });
	    ...
	    //check if user added this site to don't show again for 30 days

	  }, {
	    key: 'checkIsHidden',
	    value: function checkIsHidden(_ref) {
	      var hiddenSitesList = _ref.hiddenSitesList,
	          domain = _ref.domain;

	      var item = hiddenSitesList.find(function (item) {
	        return item.domain == domain;
	      });

	      if (item && item.time) {
	        var time = new Date(item.time);
	        var time30days = new Date(time.setDate(time.getDate() + 30));
	        var now = new Date();
	    ...
	    //overlay notification needs to be shown only once in 24 hours
	    //overlay tip notification needs to be shown once a month

	  }, {
	    key: 'checkLastDisplay',
	    value: function checkLastDisplay(_ref2) {
	      var domain = _ref2.domain,
	          type = _ref2.type,
	          lastDisplay = _ref2.lastDisplay;

	      var now = new Date().getTime();
	      var last = lastDisplay[type];
	      ...
	      switch (type) {
	        case 'overlay':
	          ...
	          if (last && last + 1000 * 60 * 60 * 24 > now) {
	            return false;
	          }
	          break;

	        case 'tip':
	          if (last + 1000 * 60 * 60 * 24 * 30 > now) {
	            return false;
	          }
	          break;
	      }
	    ...
	    key: 'updateLastDisplay',
	    value: function updateLastDisplay(_ref3) {
	      var domain = _ref3.domain,
	          type = _ref3.type;

	      _PBStorageSync2.default.pb_lastNotificationDisplay.update(function () {
	        var display = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

	        switch (type) {
	          case 'overlay':
	            display[type] = display[type] || {};
	            display[type][domain] = new Date().getTime();
	            break;
	    ...
	    key: 'updateDontShow',
	    value: function updateDontShow(_ref4) {
	      var domain = _ref4.domain;

	      _PBStorageSync2.default.doNotShowNotifyList.update(function (doNotShowList) {
	        doNotShowList = doNotShowList.filter(function (item) {
	          return item.domain != domain;
	        });
	        doNotShowList.push({
	          domain: domain,
	        ...
	        return doNotShowList;
	      });
	    }
	  }]);

	  return NtfShowManager;
	}();

	exports.default = NtfShowManager;
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Notification = function () {
	  function Notification() {
	    ...
	    this.queue = [];
	    this.props = props;
	    this.notificationSize = 'maximized';

	    _PBStorageSync2.default.pb_notification_size.get().then(function (data) {
	      if (data) {
	        _this.notificationSize = data.size;
	      }
	    });

	    window.addEventListener('message', function (event) {
	      if (chrome.runtime.getURL('').indexOf(event.origin) == 0 && event.data.toastId == _this.toastId) {
	        var data = event.data;

	        switch (data.action) {
	          case 'pb-message-display':
	            _this.options.onDisplay && _this.options.onDisplay();

	            _this.props.onDisplay.call(_this, { source: data.source });
	            break;

	          case 'message-resize':
	            _this.toast.toggleClass('pb-toast-minimized pb-toast-maximized');
	            _this.notificationSize = data.resize;

	            _PBStorageSync2.default.pb_notification_size.set({ size: data.resize });

	            _this.props.onResizeClick.call(_this, { size: data.resize });
	            break;

	          case 'message-hide':
	            _this.props.onHideClick.call(_this, { source: data.source });

	            _this.queueFade(100);
	            break;

	          case 'pb-message-close':
	            _this.userGeneratedClose = true;

	            //remove immediately
	            if (data.quickClose) {
	              _this.remove();
	            } else {
	              _this.queueFade(100, _this.options.onFadeOut);
	            }

	            if (data.triggerEvent) {
	              _this.options.onClose && _this.options.onClose();
	              _this.options.onButtonClick && _this.options.onButtonClick({ id: 'close' });

	              _this.props.onButtonClick.call(_this, { source: data.source, id: 'close' });
	              _this.props.onCloseClick.call(_this, { source: data.source });
	            }
	            break;

	          case 'pb-message-btn-click':
	            if (_this.options.buttons[data.id]) {
	              _this.options.buttons[data.id]();
	            }

	            _this.options.onButtonClick && _this.options.onButtonClick({ id: data.id });
	            _this.props.onButtonClick.call(_this, { source: data.source, id: data.id });
	            break;

	          case 'pb-message-tooltip-over':
	            if ((0, _utils.isI18N)(data.id)) {
	          ...
	          case 'pb-message-rating':
	            _this.remove();

	            if (_this.options.onRating) {
	              _this.options.onRating && _this.options.onRating(data.score);

	              _this.props.onButtonClick.call(_this, { source: data.source, id: 'rating' });
	            }
	            break;

	          case 'pb-message-share':
	            if (_this.options.onShare) {
	              _this.options.onShare && _this.options.onShare(data.media);
	              _this.props.onButtonClick.call(_this, { source: data.source, id: 'rating' });

	              if (data.media == 'link') {
	                _this.showLinkCopied(data.position);
	              }
	            }
	            break;
	        }
	    ...
	    //when focus out it stops css animation so even after 5 seconds 
	    //the notification stays and when focus back it quickly disapears 
	    //this is a fix to this
	    document.addEventListener('visibilitychange', function (event) {
	      if (document.visibilityState == 'visible') {
	        var now = new Date().getTime();

	        if (_this.queueFadeTime && now > _this.queueFadeTime + 5000) {
	          _this.remove();
	        }
	      }
	    }, false);
	  ...
	  _createClass(Notification, [{
	    key: 'addQueue',
	    value: function addQueue(options) {
	      this.queue.push(options);

	      if (!this.isNotificationActive) {
	        this.show(this.getNextQueueItem());
	      }
	    }
	  }, {
	    key: 'getNextQueueItem',
	    value: function getNextQueueItem() {
	    ...
	    value: function show(options) {
	      ...
	      if (options.quickOpen && !document.body) {
	        setTimeout(function () {
	          _this2.show(options);
	        }, 50);
	    ...
	    value: function render() {
	      ...
	      var data = this.getIframeData();

	      this.toast = (0, _jQuery2.default)('<iframe id="pb-toast-main" class="' + this.getCSSClass() + '">').on('load', function () {
	        if (_this3.options.quickOpen) {
	          _this3.toast.show().addClass('pb-toast-main-show');
	        } else {
	          _this3.toast.show().addClass('pb-toast-main-move');
	        }

	        if (!_this3.options.persistent) {
	          _this3.queueFadeTime = new Date().getTime();
	          _this3.queueFade(5000, _this3.options.onFadeOut);
	        }
	        ...
	        if (!_this3.userGeneratedClose && !_this3.options.persistent) {
	          _this3.queueFade(5000, _this3.options.onFadeOut);
	        }
	      }).on('transitionend', function () {
	        if (!_this3.toast.is('.pb-toast-main-move')) {
	          _this3.remove();
	        }
	      }).attr('src', chrome.runtime.getURL('message.htm?' + data)).appendTo('body');
	    }
	  }, {
	    key: 'getCSSClass',
	    value: function getCSSClass() {
	      return 'pb-toast-main pb-toast-' + (this.options.size || this.notificationSize) + ' ' + (this.options.cssClass || '');
	    }
	  }, {
	    key: 'init',
	    value: function init(options) {
	      ...
	      this.toastId = (0, _utils.GUID)();
	      this.options = options;
	      this.userGeneratedClose = false;
	      this.queueFadeTime = null;
	    }
	  }, {
	    key: 'remove',
	    value: function remove() {
	      ...
	      if (this.isNotificationActive) {
	        ...
	        if (!this.options.stopQueue && this.queue.length) {
	          this.show(this.getNextQueueItem());
	        }
	      }
	    }
	    ...
	    key: 'queueFade',
	    value: function queueFade(time, onFadeOut) {
	      var _this4 = this;

	      this.fid = setTimeout(function () {
	        _this4.toast.attr('class', _this4.getCSSClass());
	    ...
	    key: 'cancelFade',
	    value: function cancelFade() {
	      if (!this.toast.is('.pb-toast-main-move')) {
	        this.toast.addClass('pb-toast-main-move');
	      }
	    ...
	    key: 'showTooltip',
	    value: function showTooltip(_ref) {
	      var id = _ref.id,
	          position = _ref.position;

	      var toastPosition = this.toast.get(0).getBoundingClientRect();
	      var tooltipPosition = {
	    ...
	    value: function hideTooltip() {
	      (0, _jQuery2.default)('#pb_jq_tipsWrapper').remove();
	    }
	  }, {
	    key: 'renderTooltip',
	    value: function renderTooltip(id) {
	      this.hideTooltip();

	      return (0, _jQuery2.default)('<div id="pb_jq_tipsWrapper">' + (0, _utils.getI18N)(id) + '</div>').appendTo('body');
	    }
	  }, {
	    key: 'showLinkCopied',
	    value: function showLinkCopied(iconPosition) {
	      var toastPosition = this.toast.get(0).getBoundingClientRect();
	      var position = {
	        left: toastPosition.left + iconPosition.left,
	        top: toastPosition.top + iconPosition.top
	      };

	      (0, _jQuery2.default)('#pb-link-copied-message').remove();
	      var msg = (0, _jQuery2.default)('<div id="pb-link-copied-message">' + (0, _utils.getI18N)('NTF_LinkCopied') + '</div>').css({
	        top: position.top + 31,
	      ...
	      setTimeout(function () {
	        return msg.animate({ opacity: 0 }, 250, function () {
	          return msg.remove();
	        });
	      }, 2000);
	    }
	    ...
	    key: 'hideLinkCopied',
	    value: function hideLinkCopied() {
	      (0, _jQuery2.default)('#pb-link-copied-message').remove();
	    }
	  }, {
	    key: 'getIframeData',
	    value: function getIframeData() {
	      var data = {
	        id: this.toastId,
	        title: this.options.title,
	        subTitle: this.options.subTitle || '',
	        icon: this.options.icon != undefined ? this.options.icon : chrome.runtime.getURL('/images/logo-20x20.svg'),
	        buttons: Object.keys(this.options.buttons || {}),
	        showHide: this.options.showHide,
	        showResize: this.options.showResize,
	        source: this.options.source || '',
	        size: this.options.size || this.notificationSize || 'maximized',
	        props: {
	          domain: document.domain,
	          winArgs: this.options.winArgs || [],
	          showStars: !!this.options.showStars,
	          showShare: !!this.options.showShare
	        }
	      };
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var BlockCountManager = function () {
	    function BlockCountManager() {
	        _classCallCheck(this, BlockCountManager);
	    }

	    _createClass(BlockCountManager, [{
	        key: 'updateCounts',
	        value: function updateCounts(props) {
	            var _this = this;

	            var blockInfo = props.blockInfo,
	                _props$counts = props.counts,
	                counts = _props$counts === undefined ? 1 : _props$counts;


	            return new _Promise(function (resolve) {
	                _PBStorageSync2.default.many(_PBStorageSync2.default.pb_numOfBlocks, _PBStorageSync2.default.pb_counterBlockedSites, _PBStorageSync2.default.pb_totalBlocksByType).update(function (settings) {
	                    settings.pb_numOfBlocks++;
	                    settings.pb_totalBlocksByType = _this.updateBlockedByType({ blocks: settings.pb_totalBlocksByType, blockInfo: blockInfo, counts: counts });

	                    var curDomain = document.domain;
	                    var pb_BC = settings.pb_counterBlockedSites || {};
	                    var data = pb_BC[curDomain] || { todayCount: 0, count: 0, currentTimeRange: _this.currTR() };

	                    data.lastUpdated = Date.now();
	                    data.count = data.count + counts;

	                    if (_this.isBetweenTimeRange(data.currentTimeRange)) {
	                        data.todayCount = data.todayCount + counts;
	                    } else {
	                        data.currentTimeRange = _this.currTR();
	                        data.todayCount = counts;
	                    }

	                    settings.pb_counterBlockedSites = (pb_BC[curDomain] = data) && pb_BC;

	                    resolve({
	                        numOfBlocks: settings.pb_numOfBlocks,
	        ...
	        key: 'currTR',
	        value: function currTR() {
	            var now = new Date();
	            var todayStart = new Date(now.getFullYear(), now.getMonth(), now.getHours() >= 0 && now.getHours() < 5 ? now.getDate() - 1 : now.getDate(), 5, 0, 1); //day starts from 5PM
	            var todayEnd = new Date(todayStart.getFullYear(), todayStart.getMonth(), todayStart.getDate() + 1, 5, 0, 0); //day end at 5PM next day

	            return [todayStart.getTime(), todayEnd.getTime()];
	        }
	    }, {
	        key: 'isBetweenTimeRange',
	        value: function isBetweenTimeRange(dateRange) {
	            var now = new Date();
	            var start = new Date(dateRange[0]);
	            var end = new Date(dateRange[1]);
	        ...
	        key: 'updateBlockedByType',
	        value: function updateBlockedByType(_ref) {
	            var blocks = _ref.blocks,
	                blockInfo = _ref.blockInfo,
	                counts = _ref.counts;

	            blocks = blocks || { 'win': 0, 'tab': 0, 'overlay': 0 };

	            if (blockInfo.block == 'pop') {
	                if (blockInfo.args && blockInfo.args['2'] && /(?:(?:width\=)|(?:height\=))/.test(blockInfo.args['2'])) {
	                    blocks['win']++;
	                } else {
	                    blocks['tab']++;
	                }
	            } else {
	                blocks['overlay'] = blocks['overlay'] + (counts || 1);
	            }
	    ...
	    return BlockCountManager;
	}();

	exports.default = BlockCountManager;
...
/***/ (function(module, exports) {
	...
	var dba = window.addEventListener.bind(window),
	    locateHref = function locateHref(e) {
	  ...
	  if (e.href) {
	    ...
	    return locateHref(e.parentNode || e.parentElement);
	  }
	},
	    onRightClick = function onRightClick(e) {
	  var href = locateHref(e.target);
	  if (href) {
	    cs({ ahref: href });
	  }
	},
