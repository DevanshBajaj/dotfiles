...
/***/ (function(module, exports, __webpack_require__) {
	...
	var create_list = function create_list(options) {
	  var validurlregexp = /^((https?|ftp):\/\/)?(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i;
	  ...
	  _tag = _tag + "Box";
	  var _listtag = options.tag + "List";
	  var _internal_data_list = [];
	  var _storaqgeKey = options.storageListKey || undefined;
	  var _save_function = options.saver || undefined;
	  var _load_function = options.loader || undefined;
	  ...
	  var add = function add() {
	    var pattern_value = $("#" + _tag).val();
	    $("#" + _tag).val("").next().prop("disabled", true);
	    var GAEventName = _tag == "whiteListBox" ? "Whitelist_add" : "Blacklist_add";
	    if (validateUrl(pattern_value)) {
	      (function () {
	        var pattern = pattern_value.replace("http://", "").replace("https://", "").split("/")[0];
	        if (pattern.length > 0) {
	          if (_internal_data_list.contains(pattern)) {
	            showErrorAndRestore($("#" + _tag), "OP_urlExistsInList", "OP_whitelist");
	          }
	          _PBStorageSync2.default.many(_PBStorageSync2.default.pb_whitelist, _PBStorageSync2.default.pb_defaultWhitelist, _PBStorageSync2.default.pb_overlaylist).get().then(function (lists) {
	            if ((0, _utils.isDomainInList)(pattern, lists.pb_defaultWhitelist)) {
	              showErrorAndRestore($("#" + _tag), "OP_urlExistsInDefaultList", "OP_whitelisted");
	            } else if ((0, _utils.isDomainInList)(pattern, lists.pb_whitelist)) {
	              showErrorAndRestore($("#" + _tag), "OP_urlExistsInList", "OP_whitelist");
	            } else if (lists.pb_overlaylist[pattern] == "black") {
	              showErrorAndRestore($("#" + _tag), "OP_urlExistsInList", "OP_overlayList");
	            } else {
	  ...
	  var remove = function remove(e) {
	    var that = $(this);
	    var domain = that.parent().find(".domain").text().trim();
	    var GAEventName = _tag == "whiteListBox" ? "Whitelist_removed" : "Blacklist_removed";
	    var index = _internal_data_list.indexOf(domain);
	    var removedDomain = _internal_data_list.splice(index, 1).join();
	  ...
	  function showErrorAndRestore(sel, errorMsg, replaceStr) {
	    sel.val("").attr("placeholder", (0, _utils.getI18N)(errorMsg).replace("%s", (0, _utils.getI18N)(replaceStr)));
	    setTimeout(function () {
	      sel.attr("placeholder", (0, _utils.getI18N)("urlExamplePlaceholder"));
	    }, 2000);
	  }

	  function renderList() {
	    $("#" + _listtag).html($.map(_internal_data_list, function (item) {
	      return item.indexOf("google.") === -1 ? "<div class='list-group-item'><img class='favicon' src='http://www.google.com/s2/favicons?domain=" + item + "'><span class=\"domain\">" + item + " </span><span class='removeButton" + _listtag + " glyphicon glyphicon-remove'></span></div>" : "";
	    }).join(""));
	  }
	  ...
	  function save() {
	    if (_save_function) {
	      ...
	      _PBStorageSync2.default[_storaqgeKey].set(_internal_data_list);
	    }
	  }
	  ...
	  function init() {
	    ...
	    _PBStorageSync2.default.many(_PBStorageSync2.default.pb_whitelist, _PBStorageSync2.default.pb_overlaylist).get().then(function (settings) {
	      if (!_load_function) {
	        _internal_data_list = settings.pb_whitelist.unique();
	      } else {
	        _load_function(settings, _internal_data_list);
	      }
	      renderList();
	    });
	  }
	...
	var overlayBlockList = function () {
	  function overlayBlockList() {
	    _classCallCheck(this, overlayBlockList);
	  ...
	  _createClass(overlayBlockList, [{
	    ...
	    value: function createItems() {
	      ...
	      _PBStorageSync2.default.pb_overlayBlockedList.get().then(function () {
	        ...
	        Object.values(list).forEach(function (item) {
	          ...
	          (_items = _this.items).push.apply(_items, _toConsumableArray(item));
	        });

	        _this.createList(_this.items);
	      });
	    }
	  }, {
	    key: "removeItem",
	    value: function removeItem(item) {
	      _PBStorageSync2.default.pb_overlayBlockedList.update(function () {
	        ...
	        var blocks = (list[domain] || []).filter(function (block) {
	    ...
	    value: function parse(item) {
	      ...
	      var isRoot = url.origin == url.href.replace(/\/$/, '');
	    ...
	    value: function createList(items) {
	      ...
	      var container = $('#blackListList');

	      items.forEach(function (item) {
	        var row = $("<div class='list-group-item'><img class='favicon' src='http://www.google.com/s2/favicons?domain=" + item.domain + "'><span class=\"domain\">" + item.domain + " (found on " + item.url + ")</span><span class='remove glyphicon glyphicon-remove'></span></div>");
	...
	$(document).ready(function () {
	  ...
	  _PBStorageSync2.default.many(_PBStorageSync2.default.pb_hideBadge, _PBStorageSync2.default.pb_hideNotifications, _PBStorageSync2.default.pb_detectOverlays, _PBStorageSync2.default.pb_numOfBlocks, _PBStorageSync2.default.pb_disableContextMenu).get().then(function (settings) {
	  ...
	  $("[data-translate]").each(function () {
	    $(this).html((0, _utils.getI18N)($(this).data("translate")));
	  });
	  $("#badgeToggle").change(function () {
	    var show = $("#badgeToggle").prop("checked");
	  ...
	  $("#notificationToggle").change(function () {
	    var show = $("#notificationToggle").prop("checked");
	    _PBStorageSync2.default.pb_hideNotifications.set(!show);
	    sendGoogleAnalyticsEvent("Show_on_page_notification", show ? "checked" : "unchecked");
	  });
	  $("#overlaysToggle").change(function () {
	    var value = $("#overlaysToggle").prop("checked");
	    ...
	    _PBStorageSync2.default.pb_detectOverlays.set(value);
	  });
	  $("#overlayContextMenuToggle").change(function () {
	    var disabled = !$("#overlayContextMenuToggle").prop("checked");
	    ...
	    _PBStorageSync2.default.pb_disableContextMenu.set(disabled);
	  });
	...
	function saveBlack(blacklist) {
	  ...
	  _PBStorageSync2.default.pb_overlaylist.set(data);
	}

	function loadBlack(settings, blacklist) {
	  if (settings.pb_overlaylist) {
	    Object.keys(settings.pb_overlaylist).forEach(function (el) {
	...
	function feedback(body) {
	  var bugRequest = _const2.default.feedback_page + ("&subject=" + (body ? encodeURI(body) : "") + "&message=");
	...
	function setCheckStatus($selArr) {
	  $selArr.each(function (i, sel) {
	    var parentLabel = $(sel).parent(".checboxLabel");
	    var checked = $(sel).prop("checked");
