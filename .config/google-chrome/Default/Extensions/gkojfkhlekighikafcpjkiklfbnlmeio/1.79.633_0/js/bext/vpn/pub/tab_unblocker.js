...
var qw = string.qw, assign = Object.assign, chrome;
var new_unblocker_api, proxy_debug, proxy_debug_timing;
var E = new (be_backbone.model.extend({tab_unblockers: {},
    ff_proxy_for_url: {}, requests: {},
...
function firefox_redirect(req, details, proxy_str){
    ...
    req.redirected = true;
    details.ret.redirectUrl = req.url;
}

function parse_route_str(route_str, no_prot){
...
function _on_before_send_headers(details, host, proxy_str, opt){
    ...
    modified |= hdrs_rm(req_hdrs, 'x-hola-version');
    var is_http = url.startsWith('http:');
    if (is_http && req.algo!='direct_first')
    {
        modified |= hdrs_add(req_hdrs, 'X-Hola-Version',
            be_util.version()+' '+
            ((chrome&&be_util.get_product())||be_util.browser()));
    ...
    if (req.hola_routing && !req.hdrs_sent)
    {
        ...
        case 'direct_first_discover':
            ...
            if (is_http && (dfd=='direct' || host_cache.direct_first_test_req))
            {
                ...
                modified |= hdrs_add(req_hdrs, 'X-Hola-Direct-First', '1');
                if (!dfd)
                {
                    trace_req(details, 'delay '+dfd_delay+'ms');
                    req.count = dfd_tries;
                    req.hdrs = req_hdrs;
                    req.sp = send_direct_ajax(req);
                    modified |= hdrs_add(req_hdrs, 'X-Hola-Delay',
                        dfd_delay.toString());
                }
                break;
            }
            ...
            req.sp = send_direct_ajax(req);
            if (is_http)
                modified |= hdrs_add(req_hdrs, 'X-Hola-Direct-Discover', '1');
            break;
...
function _on_headers_received2(details, host){
    ...
    if (cmd.proxy)
    {
        // if the proxy is hdrs_only, it returned a 302 and will rerun as full
...
function _on_headers_received(details, host){
    ...
    {
        req.test_direct_block = {code: res.statusCode, headers: res.headers};
        req.route_opt.algo = null;
        be_lib.perr_ok({id: 'be_direct_first_redirect',
            info: {url: req.url, code: res.statusCode,
    ...
    if (req.test_direct_block)
    {
        ...
        {
            be_lib.perr_ok({id: 'be_direct_first_proxy_ok',
                info: {url: req.url, code: res.statusCode},
                rate_limit: {count: 1}});
...
function _on_before_request2(details, opt){
    ...
    function gen_proxy_req(req, cmd){
        ...
        if (!chrome)
        {
            E.ff_proxy_for_url[url] = assign({str: proxy_str,
    ...
    // XXX bahaa: treat direct_first as direct for old firefox (!chrome)
    var cmd = req.strategy(req.direct_resp, req.proxy_resp);
    if (!cmd)
...
function _on_before_request(details, host, route_str, opt){
    ...
    var req = E.requests[details.requestId];
    if (req && req.redirected && req.url!=details.url
        && req.redirect_route_str)
    {
        delete E.requests[details.requestId];
        if (!route_str)
            route_str = req.redirect_route_str;
        req = null;
    }
    ...
    if (!req)
    {
        req = E.requests[details.requestId] = {
            ...
            url: url,
            host: host,
            route_opt: parse_route_str(route_str),
            method: details.method,
            opt: opt,
        };
    ...
    if (!chrome)
    {
        ...
        E.ff_proxy_for_url[url] =
            {route_opt: route_opt, t: ff_proxy_timer(url)};
        return;
    }
    ...
    if (route_opt.algo=='direct_first')
    ...
    if (req.redirected)
        return;
    be_pac.set_proxy_for_url(details.url, proxy_str);
    firefox_redirect(req, details, route_str);
}
...
var on_before_request = ff_cb_wrapper('on_before_request', function(d){
    ...
    tab_unblocker_cb(d = assign({}, d, {handler: new_unblocker_api ?
...
var on_before_send_headers = ff_cb_wrapper('on_before_send_headers',
    ...
    if (trace)
    {
        ...
        hdrs_add(d.ret.requestHeaders, 'X-Hola-Request-Id', d.requestId);
    }
    return d.ret;
});
...
var on_headers_received = ff_cb_wrapper('on_headers_received', function(d){
    ...
    if (trace)
    {
        ...
        hdrs_add(d.ret.responseHeaders, 'X-Hola-Unblocker-Bext',
            build_trace_string(d.requestId, trace));
        hdrs_add(d.ret.responseHeaders, 'X-Hola-Request-Id', d.requestId);
    }
    return d.ret;
});
...
var on_completed = ff_cb_wrapper('on_completed', function(d){
    ...
    tab_unblocker_end_cb(d = assign({}, d, {cbname: 'on_completed',
        ret: {}}));
    return d.ret;
});

var on_error_occurred = cb_wrapper(function(d){
    tab_unblocker_end_cb(assign(d, {cbname: 'on_error_occurred', ret: {}}));
    return d.ret;
});
...
function tab_unblocker_cb(details){
    ...
    if (req = E.internal_reqs[url])
    {
        ...
        if (!chrome || req.reqid==details.requestId)
        {
            ...
            route_str = gen_route_str(assign({},
...
var ff_find_proxy_for_url = ff_cb_wrapper('ff_find_proxy_for_url',
    ...
    if (proxy.str)
        return proxy.str;
    return build_proxy_string(proxy);
});

function build_proxy_string(req){
    var agents;
    ...
    return req.route_opt.prot+' '+req.agent.ip+':'+req.agent.port;
}
...
function get_rule_info_from_url(url, root_url){
    ...
    root_url = (root_url||'').replace('.trigger.hola.org');
...
var tab_unblocker_add = cb_wrapper(function(tabid, rule_info, tab_url,
    ...
    E.sp.spawn(etask([function(){
        ...
        return this.loop(etask.sleep(100));
    }, function(){
        if (fix_url)
        {
...
var on_tab_updated = cb_wrapper(function(o){
    ...
    if (!chrome && version_util.cmp(be_util.version(), '1.1.834')>=0)
        return;
    if (!info || !info.url)
        return;
    ...
    return etask([function(){
        ...
        for (var r in rules.unblocker_rules)
        {
            ...
            [true, false].forEach(function(b){
                var s = gen_route_str({country: rule.country, peer: b})
                    .toLowerCase();
